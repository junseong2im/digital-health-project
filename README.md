...
[연구 과정·표·그래프를 노트북으로 보기](notebooks/our_metabolic_screening_research.ipynb)

설명 포함 42개 셀, 코드 20개 셀을 순서대로 실행하고 그래프 8개와 표 출력을 저장했다(2026-09-22 재실행 확인). GitHub에서 파일을 열면 저장된 셀 결과를 볼 수 있다. 집계 자료를 내장하여 원자료·API 키·네트워크 없이 기본 셀을 재실행할 수 있으며, 개인별 원자료나 모델 체크포인트는 포함하지 않는다. 학습 재실행은 마지막 셀에서 기본적으로 꺼져 있다. 실행 환경에는 `numpy`, `pandas`, `matplotlib`, `ipykernel`이 필요하다.
